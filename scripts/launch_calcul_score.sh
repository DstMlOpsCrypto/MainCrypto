...
echo "Conteneur trouvé : $CONTAINER_ID. Connexion et lancement de du calcul du score..."

# Exécuter les commandes dans le conteneur trouvé
docker exec -i $CONTAINER_ID /bin/bash -c "
    cd /app/scripts && \
    echo 'Dossier de scripts atteint, lancement du calcul du score...' && \
    python3 evaluate_model.py --currency='BTC-USD'
"

# Vérification du succès de la commande
if [ $? -eq 0 ]; then
    echo "Calcul du score lancé avec succès."
else
